...
void ADemoCharacter::BeginPlay()
{
	...
	GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &ADemoCharacter::OnBeginOverlap);
...
void ADemoCharacter::Tick(float DeltaTime)
{
	...
	if (Power <= 0) {
		if (!bDead) {
			...
			GetWorldTimerManager().SetTimer(UnusedHandle, this, &ADemoCharacter::RestartGame, 3.0f, false);
...
void ADemoCharacter::MoveForward(float Axis)
{
...
void ADemoCharacter::MoveRight(float Axis)
{
	if (!bDead)
	{
		const FRotator Rotation = Controller->GetControlRotation();
...
void ADemoCharacter::RestartGame()
{
...
void ADemoCharacter::OnBeginOverlap(UPrimitiveComponent* HitComp, 
